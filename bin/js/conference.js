...
onBistriConferenceReady = function() {
    ...
    if (!bc.isCompatible()) {
        ...
        alert("Votre navigateur n'est pas compatible pas WebRTC ! Esatus.fr est compatible avec Firefox, Chrome");
    ...
    bc.init({
        appId: "34b9e5d6",
        appKey: "2ba20fa295661e02705d171477e633de",
        userId: userId,
        userName: userName,
        debug: true
    });
    ...
    bc.streams.bind("onStreamClosed", function(stream) {
        ...
        if (room) {
            ...
            $.ajax({
                type: "POST",
                url: "/callsVideo/callend",
                data: {service_id: remoteUserId, user_id: userId},
    ...
    bc.signaling.bind("onPresence", function(result) {
        if (result.presence != "offline") {
            ...
            alert("L'utilisateur que vous essayez de joindre est actuellement déconnecté");
        }
    });
    ...
    bc.signaling.bind("onIncomingRequest", function(request) {
        ...
        if (confirm(request.name + " essaye de vous joindre. Cliquez sur \"Ok \" pour accepter l'appel.")) {
