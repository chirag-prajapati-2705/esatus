...
class CallsController extends Controller {
                        ...
                        if (substr($card->mark, 0, 3) == '000') {
                            ...
                            if (($preautorisation) && (date("Y-m-d H:i:s", strtotime("$preautorisation->date +7 day")) > date('Y-m-d H:i:s'))) {
                                ...
                                $params = array(
                                    ...
                                    'MONTANT' => '10000', // 80.00 €
                                    'DEVISE' => '978',
                                    'REFERENCE' => 'M-' . $service->profile_id . '-' . $this->Session->profile('id'),
                                ...
                                if ($response->codereponse == '00000') {
                                    ...
                                    Mailer::failedAutorisation($user->User->id);
    ...
    public function check_end($session_id) {
        ...
        for ($i = 0; $i < 10000; $i++) {
            ...
            sleep(10);
            if ($calld) {
                $availabilities = $this->Availability->findOneBy(array('conditions' => array('service_id' => $calld->Calldetail->serviceid)));
                if ($availabilities) {
                ...
                $CheckCall = $this->Call->findOneBy(array('conditions' => array('session_id' => $oldsession, 'payment' => 1)));
                if(!$CheckCall) {
                    $this->server_response($oldsession, $calld->Calldetail->userid, $calld->Calldetail->serviceid, $calld->Calldetail->disposition, $calld->Calldetail->disposition2, $calld->Calldetail->billsec, $calld->Calldetail->answer, $calld->Calldetail->end, $calld->Calldetail->dst);
                }
                break;
            }
        }
    ...
    public function server_response($oldsession, $userid, $serviceid, $disposition, $disposition2, $billsec, $callstart, $callend, $dst) {
        ...
        if (($dst == '#') && ($disposition == "NO ANSWER")) {
            ...
            Availabilities::ajax_empty($serviceid);
            // Envoi du mail
            Mailer::failed($mail);
        }
        ...
        if ($dst == '#' && $disposition == "ANSWERED") {
            ...
            if ($card) {
                ...
                $params = array(
                    ...
                    'DEVISE' => '978',
                    'REFERENCE' => 'M-' . $service->profile_id . '-' . $user->profile_id,
