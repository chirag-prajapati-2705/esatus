<div class="container masterhead">
    <div class="demo-headline">
        ...
        <p id="phraseCategorie"><?=  $subtext ?></p>
        ...
        <div class="search-filters row">
            ...
            <span class="layout-switch span6 text-left">
                <a class="btn btn-info btn-large active" title="Affichage en liste" href="#list"><i class="icon-list"></i></a>
                <a class="btn btn-info btn-large" title="Affichage en grille" href="#grid"><i class="icon-th"></i></a>
            </span>
        </div>
    </div>
</div>
...
<div class="container grid">
    <div class="row" style="float: left; max-width:960px; width: 100%; !important; margin: 0 auto;clear: both;">
        <?php foreach ($services as $k => $v): ?><?php $v = current($v); ?>
            <div class="col-md-6">
                <div class="expert expert-grid span3 text-center" style="word-wrap:break-word; height:168px; border:1px solid #d8d8d8;-moz-border-radius:7px;-webkit-border-radius:7px;border-radius:13px;width:95%">
                    ...
                    <div style="float:rigth;" >
                        <span class="title"><a style="color:#0096a8" href="<?= Router::url('services/view/cat:' . $v->category . '/subcat:' . $v->subcategory . '/' . $v->url); ?>"><?= ($v->username == '') ? $v->user->first_name . ' ' . $v->user->last_name : utf8_decode($v->username); ?></a></span>
                        ...
                        <?php endif; ?>
                        <h6 style="color:#aaa;margin-top:5x;font-size:0.85em;height:40px;word-wrap:break-word;"><?= $v->title; ?>
                        </h6>
                        ...
                        <?php foreach ($reviews as $k => $v): ?><?php $v = current($v); ?>
                            ...
                            <?php if ($slug == $v->category->title): ?>
                                <li >
                                    ...
                                    echo $mois, " ";
                                    echo $date[0];
                                    ?> : <?= $v->user->first_name; ?> a consulté <a href="<?= Router::url('services/view/cat:' . $v->category->slug . '/subcat:' . $v->subcategory->slug . '/slug:' . clean($v->username) . '/id:' . $v->service->id); ?>"><?= $v->username; ?>, <?= $v->service->title; ?></a> : <?= $v->comment; ?>
                                </li>
                            <?php endif; ?>
    <?php endforeach; ?>
<?php endif; ?>
